...
function Question3() {
  ...
  const { setDataAnalystScore } = useScore();
  const [selectedRatings, setSelectedRatings] = useState([]);
  const [question_list, setQuestion] = useState([]);

  const fetchQuestion = async () => {
    try {
      const response = await axios.get("http://localhost:3000/api/Question/3")
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(selectedRatings).length === question_list.length) {
      ...
      setDataAnalystScore(suitabilityPercentage.toFixed(2));
      navigate("/question4");
