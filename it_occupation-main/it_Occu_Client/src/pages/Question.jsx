...
function Question() {
  ...
  const { setItSupportScore } = useScore();
  const [selectedRatings, setSelectedRatings] = useState([]);
  const [question_list, setQuestion] = useState([]);

  const fetchQuestion = async () => {
    try {
      const response = await axios.get("http://localhost:3000/api/Question/1")
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(selectedRatings).length === question_list.length) {
      ...
      toast.success(`เปอร์เซ็นต์ความเหมาะสม: ${suitabilityPercentage.toFixed(2)}%`);
      setItSupportScore(suitabilityPercentage.toFixed(2));
      navigate("/question2");
      <Question2 score_1 ={suitabilityPercentage.toFixed(2)} />
            ...
            {question_list.map((data, key) => (
              <div key={key} className="flex w-full flex-col my-5 transition-opacity duration-500">
                <div className="card bg-white shadow-xl border-2 rounded-box grid p-12 place-items-center">
                  <div className={`text-4xl text-center text-black ${selectedRatings[data.id] ? "opacity-35" : "opacity-100"} `}>
                    {data.question}
                  </div>
                  {selectedRatings[data.id] ? (
                    <div className="text-green-300 font-bold ">เลือกเเล้ว</div>
                  ) : null}
                  <div className={`rating rating-lg flex justify-center my-2 gap-2 ${selectedRatings[data.id] ? "opacity-35" : "opacity-100"}`}>
                        ...
                        return (
                          <div key={key} className="flex flex-col items-center">
                            ...
                            <input
                              type="radio"
                              name={`radio-${data.id}`}
                              className={`radio ${sizeClasses[key]} bg-white rounded-full checked:bg-green-500`}
